...
class song_queue(asyncio.Queue):
...
class music(commands.Cog):
    ...
    async def on_ready(self):
        print('Music cog loaded') #Checking to make sure this cog is loaded upon initialization.

    def get_voice_state(self, ctx: commands.Context):
        state = self.voice_states.get(ctx.guild.id)
        if not state:
            state = voice_state(self.bot, ctx)
